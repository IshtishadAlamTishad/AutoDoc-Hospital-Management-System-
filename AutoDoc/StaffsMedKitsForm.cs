...
namespace AutoDoc
{
    public partial class StaffsMedKitsForm : Form
    {
        public StaffsMedKitsForm()
        {
            InitializeComponent();
            getRecords();
            FILLDGV();
        ...
        private void updateMedBtn_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Please select Kits for update!!", "Select Please!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            getRecords();
        }
        ...
        private void removeMedBtn_Click(object sender, EventArgs e)
        {
            if (mID > 0)
            {
                ...
                cmd.ExecuteNonQuery();
                sqlCon.Close();
                MessageBox.Show(medNameTxt.Text + " kit, " + medNameTxt.Text + " deleted successfully in database", "Deleted", MessageBoxButtons.OK, MessageBoxIcon.Information);
                getRecords();
                resetAll();
            }
            else
        ...
        private void StaffsMedKitsForm_Load(object sender, EventArgs e)
        {
