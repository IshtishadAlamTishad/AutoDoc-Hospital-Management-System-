    ...
    public partial class AdminMedicineForm : Form
    {
        ...
        private void removeMedBtn_Click(object sender, EventArgs e)
        {
            if (mID > 0)
            {
                ...
                cmd.ExecuteNonQuery();
                sqlCon.Close();
                MessageBox.Show(medNameTxt.Text + " user, " + medNameTxt.Text + " deleted successfully in database", "Deleted", MessageBoxButtons.OK, MessageBoxIcon.Information);
                getRecords();
                resetAll();
            }
            else
            {
                MessageBox.Show("Please select user for Delete!!", "Select Please!", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
